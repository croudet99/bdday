...
const T = { ink:'#1d1d1f', sub:'#6e6e73', pink:'#FF375F', orange:'#FF9F0A', indigo:'#5E5CE6', blue:'#007AFF', green:'#34C759' }
const GRAD = 'linear-gradient(135deg,#FF375F 0%,#FF6250 45%,#FF9F0A 100%)'
const GLOBE_BG = 'radial-gradient(120% 120% at 50% 0%, #141a35 0%, #070a16 68%)'
const FIELD = 'w-full rounded-xl bg-black/[0.04] px-3.5 py-2.5 text-[15px] text-[#1d1d1f] placeholder:text-black/35 border border-transparent focus:bg-white focus:border-black/10 focus:outline-none focus:ring-4 focus:ring-black/5 transition'
...
/* ========== primitives ========== */
function Card({ className = '', children, style, dark = false }) {
  return <div style={style} className={`rounded-[28px] ${dark ? 'text-white' : 'bg-white hairline'} shadow-soft ${className}`}>{children}</div>
}
function Button({ variant = 'primary', className = '', children, ...props }) {
  ...
  const map = {
    primary: 'text-white px-5 py-2.5 hover:brightness-[1.05] shadow-sm',
    dark: 'bg-[#1d1d1f] text-white px-5 py-2.5 hover:bg-black',
    soft: 'bg-black/[0.06] text-[#1d1d1f] px-5 py-2.5 hover:bg-black/[0.1]',
    ghost: 'text-[#1d1d1f] px-3 py-2 hover:bg-black/[0.05]',
  }
...
function IconButton({ className = '', children, ...props }) {
  return <button {...props} className={`w-10 h-10 rounded-full flex items-center justify-center text-[#1d1d1f] hover:bg-black/[0.06] transition ${className}`}>{children}</button>
}
...
function Segmented({ value, onChange, options, className = '' }) {
  return (
    <div className={`inline-flex items-center gap-1 rounded-full bg-black/[0.05] p-1 ${className}`}>
      {options.map((o) => (
        <button key={o.v} onClick={() => onChange(o.v)}
          className={`px-3.5 py-1.5 rounded-full text-[13px] font-semibold transition whitespace-nowrap ${value === o.v ? 'bg-white text-[#1d1d1f] shadow-sm' : 'text-[#6e6e73] hover:text-[#1d1d1f]'}`}>{o.label}</button>
      ))}
    </div>
  )
}
...
function Landing({ publicBirthdays, onStart }) {
  ...
  const Feature = ({ icon: Icon, tint, title, desc }) => (
    <Card className="p-6 flex flex-col justify-between">
      <div className="w-11 h-11 rounded-2xl flex items-center justify-center" style={{ background: tint + '1a', color: tint }}><Icon className="w-5 h-5" /></div>
      <div className="mt-6"><p className="font-semibold text-[17px]">{title}</p><p className="text-[14px] text-[#6e6e73] mt-1 leading-snug">{desc}</p></div>
    </Card>
  )

  return (
    <div>
      <nav className="sticky top-0 z-30 glass border-b border-black/[0.06]">
        <div className="max-w-6xl mx-auto px-5 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <Button variant="dark" onClick={onStart}>Get started</Button>
          </div>
        </div>
      </nav>

      <section className="max-w-6xl mx-auto px-5 pt-16 pb-10 text-center">
        <div className="inline-flex items-center gap-1.5 rounded-full bg-black/[0.05] px-3 py-1 text-[13px] font-medium text-[#6e6e73] mb-5">🌍 The world&apos;s birthday calendar</div>
        ...
        <p className="mt-5 text-[19px] text-[#6e6e73] max-w-2xl mx-auto leading-relaxed">Pin your birthday to a live 3D globe, follow friends across the planet, and get a beautifully-timed email before every birthday you love.</p>
      ...
      <section className="max-w-6xl mx-auto px-5 pb-20">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:auto-rows-[188px]">
          <Card dark className="md:col-span-2 md:row-span-2 overflow-hidden relative" style={{ background: GLOBE_BG }}>
            <div className="absolute top-5 left-6 z-10">
              <p className="text-[13px] text-white/60">Live now</p>
              <p className="font-semibold text-lg">Celebrating today</p>
            </div>
            <div className="absolute top-5 right-6 z-10 text-right"><p className="font-display text-4xl font-bold">{todays.length}</p><p className="text-[12px] text-white/50">around the world</p></div>
          ...
          <Feature icon={Heart} tint={T.indigo} title="Follow anyone" desc="Subscribe to friends worldwide and never forget." />
          <Card className="p-6 flex items-center justify-between">
            <div><p className="font-display text-4xl font-bold">{upcoming}</p><p className="text-[14px] text-[#6e6e73] mt-1">birthdays in the next 7 days</p></div>
            <div className="w-11 h-11 rounded-2xl flex items-center justify-center" style={{ background: T.orange + '1a', color: T.orange }}><CalendarClock className="w-5 h-5" /></div>
          </Card>
          <Feature icon={Sparkles} tint={T.blue} title="Private mode" desc="Track family & friends privately — never shown publicly." />
        </div>
      </section>
    </div>
  )
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      <Card className="w-full max-w-[400px] p-8">
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <p className="text-[15px] text-[#6e6e73] mt-1">{mode === 'signup' ? 'Join the global birthday calendar' : 'Sign in to your Birthday Globe'}</p>
        </div>
        ...
        <div className="mt-5 text-center text-[14px] text-[#6e6e73]">
          {mode === 'signup' ? 'Already have an account? ' : 'New here? '}
          <button className="font-semibold" style={{ color: T.blue }} onClick={() => setMode(mode === 'signup' ? 'login' : 'signup')}>{mode === 'signup' ? 'Sign in' : 'Create one'}</button>
        </div>
        <button className="mt-3 w-full text-center text-[13px] text-black/40 hover:text-black/70" onClick={onBack}>← Back to home</button>
      </Card>
    </div>
  )
}
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-10">
      <Card className="w-full max-w-[520px] p-8">
        ...
        <p className="text-[15px] text-[#6e6e73] mb-6">This creates your pin on the global calendar.</p>
        <div className="space-y-4">
          <div><Label className="text-[13px] font-medium text-[#6e6e73]">Display name</Label><Input value={name} onChange={(e) => setName(e.target.value)} className={FIELD + ' mt-1.5'} /></div>
          <div className="grid grid-cols-3 gap-3">
            <div><Label className="text-[13px] font-medium text-[#6e6e73]">Month</Label><Select value={month} onValueChange={(v) => { setMonth(v); setDay('') }}><SelectTrigger className={FIELD + ' mt-1.5'}><SelectValue placeholder="Month" /></SelectTrigger><SelectContent>{MONTHS.map((m, i) => <SelectItem key={i} value={String(i + 1)}>{m}</SelectItem>)}</SelectContent></Select></div>
            <div><Label className="text-[13px] font-medium text-[#6e6e73]">Day</Label><Select value={day} onValueChange={setDay}><SelectTrigger className={FIELD + ' mt-1.5'}><SelectValue placeholder="Day" /></SelectTrigger><SelectContent className="max-h-60">{Array.from({ length: dayCount }, (_, i) => i + 1).map((d) => <SelectItem key={d} value={String(d)}>{d}</SelectItem>)}</SelectContent></Select></div>
            <div><Label className="text-[13px] font-medium text-[#6e6e73]">Year</Label><Input type="number" value={year} onChange={(e) => setYear(e.target.value)} placeholder="Optional" className={FIELD + ' mt-1.5'} /></div>
          </div>
          ...
          <div><Label className="text-[13px] font-medium text-[#6e6e73]">Country</Label><Select value={country} onValueChange={setCountry}><SelectTrigger className={FIELD + ' mt-1.5'}><SelectValue placeholder="Where are you?" /></SelectTrigger><SelectContent className="max-h-72">{COUNTRIES.map((c) => <SelectItem key={c.code} value={c.code}>{flagEmoji(c.code)} {c.name}</SelectItem>)}</SelectContent></Select></div>
          <div className="grid grid-cols-2 gap-3">
            <div><Label className="text-[13px] font-medium text-[#6e6e73] flex items-center gap-1"><Twitter className="w-3.5 h-3.5" /> X</Label><Input value={x} onChange={(e) => setX(e.target.value)} placeholder="@handle" className={FIELD + ' mt-1.5'} /></div>
            <div><Label className="text-[13px] font-medium text-[#6e6e73] flex items-center gap-1"><Instagram className="w-3.5 h-3.5" /> Instagram</Label><Input value={ig} onChange={(e) => setIg(e.target.value)} placeholder="@handle" className={FIELD + ' mt-1.5'} /></div>
          </div>
    ...
    <div>
      <header className="sticky top-0 z-30 glass border-b border-black/[0.06]">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between gap-3">
          ...
          <Segmented value={mode} onChange={setMode} options={[{ v: 'global', label: '🌍 Global' }, { v: 'personal', label: '🔒 Personal' }]} />
          <div className="flex items-center gap-1">
            <Popover>
              <PopoverTrigger asChild>
                <IconButton className="relative"><Bell className="w-5 h-5" />{unread > 0 && <span style={{ background: T.pink }} className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full ring-2 ring-white" />}</IconButton>
              </PopoverTrigger>
              <PopoverContent className="w-80 rounded-2xl border border-black/[0.08] shadow-float p-3" align="end">
                <div className="flex items-center justify-between mb-2 px-1"><p className="font-semibold">Notifications</p>{unread > 0 && <button onClick={markAllRead} className="text-[13px] font-medium" style={{ color: T.blue }}>Mark all read</button>}</div>
                <div className="space-y-1.5 max-h-80 overflow-auto">
                  {notifications.length === 0 && <p className="text-[14px] text-[#6e6e73] py-8 text-center">No notifications yet</p>}
                  {notifications.map((n) => (
                    <div key={n.id} className={`rounded-xl p-3 ${n.read_at ? '' : 'bg-black/[0.03]'}`}>
                      <p className="font-medium text-[14px] flex items-center gap-1.5"><Gift className="w-4 h-4" style={{ color: T.pink }} /> {n.title}</p>
                      {n.body && <p className="text-[13px] text-[#6e6e73] mt-0.5">{n.body}</p>}
                    </div>
                  ))}
                </div>
              </PopoverContent>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
          ...
          <div className="inline-flex items-center gap-1 rounded-full bg-black/[0.05] p-1">
            {tabDef.map((t) => (
              <button key={t.v} onClick={() => setTab(t.v)}
                className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-[13px] font-semibold transition whitespace-nowrap ${tab === t.v ? 'bg-white text-[#1d1d1f] shadow-sm' : 'text-[#6e6e73] hover:text-[#1d1d1f]'}`}>
        ...
        {tab === 'globe' && <GlobeTab publicBirthdays={publicBirthdays} tM={tM} tD={tD} onSelect={setSelected} followedIds={followedIds} />}
...
function GlobeTab({ publicBirthdays, tM, tD, onSelect }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <Card dark className="lg:col-span-2 overflow-hidden relative" style={{ background: GLOBE_BG }}>
        <div className="absolute top-5 left-6 z-10"><p className="text-[13px] text-white/60">Live birthday globe</p><p className="font-semibold">Pink pins celebrate today</p></div>
        <div className="pt-6"><GlobeView points={points} onPointClick={(p) => onSelect(p.data)} /></div>
      </Card>
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <Card className="p-5"><p className="font-display text-3xl font-bold" style={{ color: T.pink }}>{todays.length}</p><p className="text-[13px] text-[#6e6e73] mt-1">celebrating today</p></Card>
          <Card className="p-5"><p className="font-display text-3xl font-bold" style={{ color: T.orange }}>{week.length}</p><p className="text-[13px] text-[#6e6e73] mt-1">in the next 7 days</p></Card>
        </div>
        <Card className="p-5">
          <p className="font-semibold mb-3">Today · {MONTH_ABBR[tM - 1]} {tD}</p>
          <div className="space-y-1.5 max-h-[300px] overflow-auto">
            {todays.length === 0 && <p className="text-[14px] text-[#6e6e73] py-6 text-center">No public birthdays today.</p>}
...
function PersonRow({ b, onClick }) {
  ...
  return (
    <button onClick={onClick} className="w-full text-left flex items-center gap-3 rounded-2xl p-2 hover:bg-black/[0.03] transition">
      <Avatar name={b.display_name} size="w-10 h-10" text="text-sm" />
      <div className="min-w-0 flex-1"><p className="font-medium text-[15px] truncate">{b.display_name}</p><p className="text-[13px] text-[#6e6e73] flex items-center gap-1"><MapPin className="w-3 h-3" /> {c ? `${flagEmoji(b.country_code)} ${c.name}` : 'Earth'}</p></div>
    </button>
  )
}
...
const SRC_COLOR = { self: '#FF375F', personal: '#FF9F0A', subscribed: '#5E5CE6', public: '#5E5CE6' }
...
function UpcomingTab({ mode, publicBirthdays, followedIds, personal, profile, onSelect }) {
  ...
  return (
    <div>
      <div className="flex items-baseline gap-3 mb-5"><h2 className="font-display text-[28px] font-bold">Next 7 days</h2><span className="text-[15px] text-[#6e6e73]">{upcoming.length} coming up</span></div>
      {upcoming.length === 0 && <Card className="p-12 text-center"><div className="text-4xl mb-2">🎈</div><p className="font-semibold text-lg">No birthdays in the next week</p><p className="text-[#6e6e73] mt-1">{mode === 'global' ? 'Invite friends so the globe fills up.' : 'Follow people or add private birthdays.'}</p></Card>}
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {upcoming.map((e, i) => {
          const c = COUNTRY_MAP[e.country_code]; const clickable = e.source === 'public' || e.source === 'subscribed'
          const col = SRC_COLOR[e.source]
          return (
            <Card key={i} className="p-5">
              <div className="flex items-center justify-between mb-4">
                <span className="rounded-full px-3 py-1 text-[12px] font-semibold" style={{ background: col + '1a', color: col }}>{untilLabel(e.days)}</span>
                <span className="text-[13px] font-medium text-[#6e6e73]">{MONTH_ABBR[e.birth_month - 1]} {e.birth_day}</span>
              </div>
              <button disabled={!clickable} onClick={() => clickable && onSelect(e)} className="w-full text-left flex items-center gap-3">
                <Avatar name={e.name} />
                <div className="min-w-0"><p className="font-medium text-[15px] truncate">{e.name}</p><p className="text-[13px] text-[#6e6e73]">{e.source === 'personal' ? (e.relationship || 'Private') : c ? `${flagEmoji(e.country_code)} ${c.name}` : (e.source === 'self' ? 'Your birthday' : 'Earth')}</p></div>
              </button>
            </Card>
          )
        })}
  ...
  return (
    <Card className="p-6">
      <div className="flex items-center justify-between mb-5">
        <div><h2 className="font-display text-[24px] font-bold">{MONTHS[viewMonth - 1]}</h2><p className="text-[14px] text-[#6e6e73]">{mode === 'global' ? 'Public birthdays worldwide' : 'You, your people & subscriptions'}</p></div>
      ...
      <div className="grid grid-cols-7 gap-1.5 mb-1.5 text-center text-[12px] font-medium text-[#6e6e73]">{DOW.map((d) => <div key={d}>{d}</div>)}</div>
      <div className="grid grid-cols-7 gap-1.5">
        {cells.map((d, i) => {
          if (!d) return <div key={i} />
          const list = byDay[d] || []; const isToday = viewMonth === tM && d === tD
          return (
            <div key={i} className={`min-h-[84px] rounded-2xl p-2 ${isToday ? 'ring-2' : 'bg-black/[0.025]'}`} style={isToday ? { boxShadow: `inset 0 0 0 2px ${T.pink}` } : {}}>
              <div className={`text-[12px] font-semibold ${isToday ? '' : 'text-[#6e6e73]'}`} style={isToday ? { color: T.pink } : {}}>{d}</div>
              <div className="space-y-1 mt-1">
                {list.slice(0, 2).map((e, idx) => { const col = SRC_COLOR[e.source]; return (
                  <button key={idx} onClick={() => (e.source === 'public' || e.source === 'subscribed') && onSelect(e)} className="w-full truncate text-left text-[10px] font-medium px-1.5 py-0.5 rounded-md" style={{ background: col + '1f', color: col }}>{e.name}</button>
                )})}
                {list.length > 2 && <div className="text-[10px] text-[#6e6e73] px-1">+{list.length - 2} more</div>}
              </div>
            </div>
          )
        })}
...
function DiscoverTab({ user, followedIds, follow, unfollow }) {
  ...
  return (
    <div>
      ...
      {busy && <p className="text-[14px] text-[#6e6e73]">Searching…</p>}
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {results.map((b) => {
          ...
          return (
            <Card key={b.id} className="p-4 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1"><p className="font-medium text-[15px] truncate">{b.display_name}</p><p className="text-[13px] text-[#6e6e73] truncate">{MONTH_ABBR[b.birth_month - 1]} {b.birth_day} · {c ? `${flagEmoji(b.country_code)} ${c.name}` : 'Earth'}</p></div>
              {following ? <IconButton className="bg-black/[0.05]" onClick={() => unfollow(b.id)}><UserMinus className="w-4 h-4" /></IconButton> : <Button variant="primary" onClick={() => follow(b.id)} className="px-3.5 py-2"><UserPlus className="w-4 h-4" /></Button>}
            </Card>
          )
        })}
      </div>
      {!busy && results.length === 0 && <p className="text-[14px] text-[#6e6e73]">No public profiles found.</p>}
    </div>
  )
}
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-4">
      <Card className="p-6">
        ...
        <p className="text-[14px] text-[#6e6e73] mb-4">Only you can see these — for people not on Birthday Globe.</p>
        <div className="space-y-3">
          ...
          <Button onClick={add} variant="dark" disabled={busy} className="w-full py-3">{busy ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Add birthday'}</Button>
        </div>
      </Card>
      <div className="space-y-4">
        <Card className="p-6">
          ...
          <div className="space-y-1 max-h-64 overflow-auto">
            {personal.length === 0 && <p className="text-[14px] text-[#6e6e73] py-4 text-center">Nothing yet — add someone.</p>}
            {personal.map((p) => (
              <div key={p.id} className="flex items-center gap-3 rounded-2xl p-2 hover:bg-black/[0.03]">
                <Avatar name={p.person_name} size="w-10 h-10" text="text-sm" />
                <div className="flex-1 min-w-0"><p className="font-medium text-[15px] truncate">{p.person_name}</p><p className="text-[13px] text-[#6e6e73]">{MONTH_ABBR[p.birth_month - 1]} {p.birth_day}{p.relationship ? ` · ${p.relationship}` : ''}</p></div>
        ...
        <Card className="p-6">
          ...
          <div className="space-y-1 max-h-64 overflow-auto">
            {subscribed.length === 0 && <p className="text-[14px] text-[#6e6e73] py-4 text-center">Follow people from Discover.</p>}
            {subscribed.map((b) => (
              <div key={b.id} className="flex items-center gap-3 rounded-2xl p-2 hover:bg-black/[0.03]">
                <button className="flex items-center gap-3 flex-1 min-w-0 text-left" onClick={() => onSelect(b)}><Avatar name={b.display_name} size="w-10 h-10" text="text-sm" /><div className="min-w-0"><p className="font-medium text-[15px] truncate">{b.display_name}</p><p className="text-[13px] text-[#6e6e73]">{MONTH_ABBR[b.birth_month - 1]} {b.birth_day}</p></div></button>
...
function ProfileTab({ user, profile, setProfile, reload }) {
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-4 items-start">
      <Card className="p-6">
        <div className="flex items-center gap-3 mb-5"><Avatar name={name || user.email} size="w-14 h-14" text="text-xl" /><div><h2 className="font-display text-[22px] font-bold">Your profile</h2><p className="text-[14px] text-[#6e6e73]">{user.email}</p></div></div>
        <div className="space-y-4">
          <div><Label className="text-[13px] font-medium text-[#6e6e73]">Display name</Label><Input value={name} onChange={(e) => setName(e.target.value)} className={FIELD + ' mt-1.5'} /></div>
          <div className="grid grid-cols-3 gap-3">
            <div><Label className="text-[13px] font-medium text-[#6e6e73]">Month</Label><Select value={month} onValueChange={(v) => { setMonth(v); setDay('') }}><SelectTrigger className={FIELD + ' mt-1.5'}><SelectValue placeholder="Month" /></SelectTrigger><SelectContent>{MONTHS.map((m, i) => <SelectItem key={i} value={String(i + 1)}>{m}</SelectItem>)}</SelectContent></Select></div>
          ...
          <div><Label className="text-[13px] font-medium text-[#6e6e73]">Country</Label><Select value={country} onValueChange={setCountry}><SelectTrigger className={FIELD + ' mt-1.5'}><SelectValue placeholder="Country" /></SelectTrigger><SelectContent className="max-h-72">{COUNTRIES.map((c) => <SelectItem key={c.code} value={c.code}>{flagEmoji(c.code)} {c.name}</SelectItem>)}</SelectContent></Select></div>
          <div className="grid grid-cols-2 gap-3">
            <div><Label className="text-[13px] font-medium text-[#6e6e73] flex items-center gap-1"><Twitter className="w-3.5 h-3.5" /> X</Label><Input value={x} onChange={(e) => setX(e.target.value)} placeholder="@handle" className={FIELD + ' mt-1.5'} /></div>
            <div><Label className="text-[13px] font-medium text-[#6e6e73] flex items-center gap-1"><Instagram className="w-3.5 h-3.5" /> Instagram</Label><Input value={ig} onChange={(e) => setIg(e.target.value)} placeholder="@handle" className={FIELD + ' mt-1.5'} /></div>
          </div>
      ...
      <div className="space-y-4">
        ...
        <Card className="p-6">
          ...
          <p className="text-[14px] text-[#6e6e73] mt-1 mb-3">Choose when to get emailed before each birthday.</p>
          <div className="flex flex-wrap gap-2">
            {[{ o: 3, l: '3 days before' }, { o: 1, l: '1 day before' }, { o: 0, l: 'Morning of' }].map(({ o, l }) => {
              const on = offsets.includes(o)
              return <button key={o} type="button" disabled={!reminders} onClick={() => toggleOffset(o)} className={`inline-flex items-center gap-1.5 rounded-full px-3.5 py-2 text-[13px] font-semibold transition disabled:opacity-40 ${on ? 'text-white' : 'bg-black/[0.05] text-[#1d1d1f] hover:bg-black/[0.09]'}`} style={on ? { backgroundImage: GRAD } : {}}>{on && <Check className="w-3.5 h-3.5" />}{l}</button>
            })}
          </div>
          <div className="mt-4 pt-4 border-t border-black/[0.06]">
            ...
            <p className="text-[12px] text-[#6e6e73] text-center mt-2">Emails you a live preview of your nearest upcoming reminder.</p>
          </div>
        </Card>
      </div>
    </div>
...
function ToggleRow({ title, desc, checked, onChange, disabled }) {
  return (
    <div className="flex items-center justify-between rounded-2xl bg-black/[0.03] px-4 py-3">
      <div><p className="font-medium text-[15px]">{title}</p><p className="text-[13px] text-[#6e6e73]">{desc}</p></div>
...
function PersonDialog({ person, onClose, followedIds, follow, unfollow, meId }) {
  ...
  return (
    <Dialog open={!!person} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="rounded-[28px] border border-black/[0.08] shadow-float p-0 overflow-hidden max-w-md">
        ...
        <div className="p-6 space-y-4">
          <div className="inline-flex items-center gap-2 rounded-full bg-black/[0.05] px-3.5 py-2 text-[14px] font-medium"><Cake className="w-4 h-4" style={{ color: T.pink }} /> {MONTHS[person.birth_month - 1]} {ordinal(person.birth_day)}{age ? ` · turning ${age + 1}` : ''}</div>
          <div className="flex flex-wrap gap-2">
            {person.x_handle && <a href={`https://x.com/${person.x_handle.replace('@', '')}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-full bg-black/[0.05] px-3.5 py-2 text-[14px] font-medium hover:bg-black/[0.09] transition"><Twitter className="w-4 h-4" /> {person.x_handle}</a>}
            {person.instagram_handle && <a href={`https://instagram.com/${person.instagram_handle.replace('@', '')}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-full bg-black/[0.05] px-3.5 py-2 text-[14px] font-medium hover:bg-black/[0.09] transition"><Instagram className="w-4 h-4" /> {person.instagram_handle}</a>}
          </div>
          {!person.x_handle && !person.instagram_handle && <p className="text-[13px] text-[#6e6e73]">No socials shared.</p>}
          {!isMe && (following
...
function buildGlobePoints(publicBirthdays, tM, tD) {
  ...
  for (const b of publicBirthdays) {
    ...
    pts.push({ lat: c.lat + jLat, lng: c.lng + jLng, color: isToday ? '#FF375F' : soon ? '#FF9F0A' : '#5E5CE6', r: isToday ? 0.8 : soon ? 0.5 : 0.28, alt: isToday ? 0.1 : soon ? 0.05 : 0.01, label: `${flagEmoji(b.country_code)} ${b.display_name} · ${c.name}${isToday ? ' · Today!' : soon ? ` · in ${dU}d` : ''}`, data: b })
  }
  return pts
}
